 ...
 *
 * Renders LandingPageLight unchanged for the `light` theme. Renders the inline
 * LandingPageDark below for `dark`. The dark variant is a structural clone of
 * LandingPageLight (same sections, same hover effects, same animations, same
 * content) with the palette inverted to a dark substrate and paper-ink text,
 * plus an inverted CTA (paper material on graphite, instead of graphite on
 * paper). The hero pipeline panel runs the same cascade animation in both
 * themes (SEQ-01 → SEQ-06 active step walks the rail every 1.4 s).
 */
const LandingPage = () => {
  const { theme } = useTheme();
  if (theme === 'light') return <LandingPageLight />;
...
/* Dark palette — structural mirror of LandingPageLight `C` with substrate
 * inverted and paper-ink text. CTA is paper-on-graphite (inverse of light's
 * graphite-on-paper). Signal is a more luminous bronze that reads on dark. */
const C = {
  bg0: '#0F0F11',                          // institutional graphite substrate
  bg1: '#16161A',                          // operational layer (slightly raised)
  bg2: '#1E1E24',                          // focus layer
  bg3: '#08080A',                          // sharp / inset surface (deeper than bg0)
  border1: 'rgba(245,242,236,0.08)',       // paper-white border, low opacity
  border2: 'rgba(245,242,236,0.14)',
  border3: 'rgba(245,242,236,0.22)',
  text1: '#F5F2EC',                        // warm paper ink (primary text)
  text2: '#A09A92',                        // warm secondary
  text3: '#6B655E',                        // warm muted
  signal: '#D4A574',                       // luminous bronze on dark
  ctaBg: '#F5F2EC',                        // INVERTED: paper material as CTA
  ctaInk: '#1A1714',                       // graphite text on paper CTA
};
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundImage:
          'radial-gradient(1200px 600px at 50% -20%, rgba(212,165,116,0.04), transparent 60%)',
...
const HeavyButton = ({ children, onClick, testid, size = 'md' }) => {
  ...
  const drop = pressed
    ...
    : '0 1px 0 rgba(0,0,0,0.22), 0 10px 24px rgba(0,0,0,0.20), 0 2px 4px rgba(0,0,0,0.10)';
  return (
    <button
      data-testid={testid}
      onClick={onClick}
      ...
      style={{
        ...
        alignItems: 'center',
        gap: 8,
        boxShadow: `inset 0 1px 0 rgba(255,255,255,0.04), inset 0 -1px 0 rgba(0,0,0,0.40), ${drop}`,
  ...
  >
    <div
      ...
      style={{
        ...
        pointerEvents: 'none',
        background:
          'radial-gradient(820px 500px at 12% 16%, rgba(212,165,116,0.07), transparent 60%)',
      }}
    />
    <div
      className="hero-grid"
              ...
              <>
                <span
                  ...
                  style={{
                    ...
                    marginRight: 8,
                    boxShadow: '0 0 0 3px rgba(212,165,116,0.14)',
                  }}
                />
    ...
    <div style={{ maxWidth: 1240, margin: '0 auto' }}>
      <div
        style={{
          ...
          boxShadow:
            '0 1px 0 rgba(255,255,255,0.04), 0 28px 70px rgba(0,0,0,0.07), 0 6px 18px rgba(0,0,0,0.04)',
        }}
      >
        {/* ============ 1. HEAD ============ */}
        <div
  ...
  return (
    <div
      ...
      style={{
        position: 'relative',
        background: C.bg3,
        border: `1px solid ${hovered ? 'rgba(0,0,0,0.18)' : C.border2}`,
        borderRadius: 12,
        boxShadow: hovered
          ? '0 1px 0 rgba(255,255,255,0.04), 0 28px 60px rgba(0,0,0,0.12), 0 4px 14px rgba(0,0,0,0.06)'
          : '0 1px 0 rgba(255,255,255,0.04), 0 24px 60px rgba(0,0,0,0.10), 0 4px 14px rgba(0,0,0,0.06)',
        overflow: 'hidden',
        ...
        {PIPELINE_ROWS.map((r, i) => {
          ...
          <div
            ...
            style={{
              ...
              background:
                state === 'active' ? 'rgba(212,165,116,0.06)' : 'transparent',
    ...
    <div style={{ maxWidth: 1240, margin: '0 auto' }}>
      ...
      <div
        style={{
          ...
          overflow: 'hidden',
          boxShadow:
            '0 1px 0 rgba(255,255,255,0.04), 0 12px 40px rgba(0,0,0,0.08)',
        }}
      >
        {SEQUENCE.map((s, i) => (
    ...
    >
      ...
      <style>{`
        ...
        @keyframes evax-gold-breath {
          0%, 100% { box-shadow: 0 1px 0 rgba(255,255,255,0.04), 0 18px 44px rgba(0,0,0,0.12), 0 0 0 0 rgba(212,165,116,0.00); }
          50%      { box-shadow: 0 1px 0 rgba(255,255,255,0.04), 0 22px 52px rgba(0,0,0,0.14), 0 0 0 4px rgba(212,165,116,0.10); }
        }
      `}</style>
    </section>
  );
...
const ModePanel = ({ mode, onStart, index, revealed, hoveredId, setHoveredId }) => {
  ...
  return (
    <div
      ...
      style={{
        position: 'relative',
        background: isHovered ? C.bg2 : C.bg3,
        border: `1px solid ${
          isHovered
            ? mode.recommended
        ...
        boxShadow: isHovered
          ? mode.recommended
            ? '0 1px 0 rgba(255,255,255,0.04), 0 22px 52px rgba(0,0,0,0.14), 0 0 0 4px rgba(212,165,116,0.08)'
            : '0 1px 0 rgba(255,255,255,0.04), 0 22px 52px rgba(0,0,0,0.14)'
          : mode.recommended
            ? '0 1px 0 rgba(255,255,255,0.04), 0 14px 36px rgba(0,0,0,0.10)'
            : '0 1px 0 rgba(255,255,255,0.04), 0 8px 24px rgba(0,0,0,0.06)',
      ...
      <span
        ...
        style={{
          ...
          background: mode.recommended
            ? 'linear-gradient(90deg, rgba(160,122,46,0) 0%, rgba(212,165,116,0.85) 30%, rgba(160,122,46,1) 100%)'
            : 'linear-gradient(90deg, rgba(26,23,20,0) 0%, rgba(0,0,0,0.72) 30%, rgba(26,23,20,1) 100%)',
          ...
          style={{
            ...
            ...(isHovered
              ? {
                  ...
                  borderColor: 'rgba(212,165,116,0.60)',
                }
              : null),
          }}
        >
        ...
        {mode.points.map((p) => (
          ...
          >
            <Plus
              ...
              strokeWidth={2}
              style={{
                color: isHovered ? (mode.recommended ? C.signal : C.text1) : C.text3,
                marginTop: 4,
  ...
  return (
    <div
      ...
      style={{
        position: 'relative',
        background: C.bg3,
        border: `1px solid ${isHovered ? 'rgba(0,0,0,0.20)' : C.border1}`,
        borderRadius: 14,
        padding: 28,
        overflow: 'hidden',
          ...
          return (
            <span
              ...
              style={{
                ...
                background: isHovered ? C.bg2 : C.bg1,
                border: `1px solid ${isHovered ? 'rgba(0,0,0,0.32)' : C.border1}`,
                padding: '5px 10px',
                borderRadius: 6,
                transform: isHovered ? 'translateY(-2px)' : 'translateY(0)',
                boxShadow: isHovered
                  ? '0 4px 12px rgba(0,0,0,0.06), inset 0 0 0 1px rgba(255,255,255,0.04)'
                  : 'none',
                transition: [
...
const PORTFOLIO_STATUS_LIGHT = {
  ...
  archived:    { label: 'ARCHIVED',     Icon: Archive,      dot: C.text3 },
};
  ...
  >
    <div
      style={{
        ...
        textAlign: 'center',
        boxShadow:
          '0 1px 0 rgba(255,255,255,0.04), 0 24px 60px rgba(0,0,0,0.10)',
        position: 'relative',
        overflow: 'hidden',
      }}
    >
      <div
        ...
        style={{
          ...
          pointerEvents: 'none',
          background:
            'radial-gradient(600px 240px at 50% 0%, rgba(212,165,116,0.06), transparent 60%)',
        }}
      />
...
const Footer = () => {
  ...
  >
    ...
    <div style={{ borderTop: `1px solid ${C.border1}` }}>
      <div style={{ maxWidth: 1240, margin: '0 auto' }}>
        <FooterExtras tone="dark" mono={FONT_MONO} />
      </div>
    </div>
...
// (dark variant is internal — exported via dispatcher above)
